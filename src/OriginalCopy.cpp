...
int main()
{
  ...
  struct tm *tm_info;
  char timestamp[20];
  char a_filename[50], b_filename[50], result_filename[50];
  ...
  snprintf(a_filename, sizeof(a_filename), "%s_a.in", timestamp);
  snprintf(b_filename, sizeof(b_filename), "%s_b.in", timestamp);
  snprintf(result_filename, sizeof(result_filename), "%s.out", timestamp);

  // Salvar números a e b em arquivos .in
  FILE *fa = fopen(a_filename, "w");
  if (!fa) { perror("Erro ao abrir a.in"); exit(1); }
  for (i = d_a - 1; i >= 0; i--) fprintf(fa, "%d", a[i]);
  fclose(fa);

  FILE *fb = fopen(b_filename, "w");
  if (!fb) { perror("Erro ao abrir b.in"); exit(1); }
  for (i = d_b - 1; i >= 0; i--) fprintf(fb, "%d", b[i]);
  fclose(fb);
  ...
  // Inicializa todo o vetor inicial com 0, em caso de um numero for menor que o outro
  ...
  // Salvar resultado em .out
  FILE *fout = fopen(result_filename, "w");
  if (!fout) { perror("Erro ao abrir result.out"); exit(1); }
  int max_r = 2 * d - 1;
  while (max_r > 0 && r[max_r] == 0) max_r--;
  for (i = max_r; i >= 0; i--) fprintf(fout, "%d", r[i]);
  fclose(fout);
